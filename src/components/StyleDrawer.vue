      ...
      <div v-if="selectedComponent" class="drawer-content">
        <div v-for="(value, name) in selectedComponent.styles" :key="name" class="control">
          ...
          <div v-if="isColor(value)" class="color-control">
            <input :id="`${selectedComponent.id}-${name}-color`" v-model="selectedComponent.styles[name]" type="color" />
            <input :id="`${selectedComponent.id}-${name}`" v-model="selectedComponent.styles[name]" type="text" />
          </div>
          <input v-else :id="`${selectedComponent.id}-${name}`" v-model="selectedComponent.styles[name]" type="text" />
        </div>
      </div>

      <div v-else class="empty-state">Select a component to edit its styles.</div>

      <div class="drawer-footer">
        <button class="copy-button" type="button" :disabled="!selectedComponent" @click="copyComponent">
...
const { selectedComponent, drawerOpen, openDrawer, closeDrawer } = useStyleManager();
...
.drawer-header h3{margin:0;font-size:18px;font-weight:700}.drawer-header p{margin:4px 0 0;font-size:12px;color:#6b7280}
...
.drawer-content{flex:1;overflow-y:auto;padding:20px}.control{margin-bottom:16px}.control label{display:block;margin-bottom:7px;font-size:12px;font-weight:600;color:#374151}
...
.color-control{display:flex;gap:8px}.color-control input[type=color]{width:42px;height:38px;padding:2px;border:1px solid #d1d5db;border-radius:7px;background:#fff;cursor:pointer}.color-control input[type=text]{flex:1;min-width:0}
.empty-state{flex:1;padding:24px 20px;color:#6b7280;font-size:13px}.drawer-footer{padding:16px 20px;border-top:1px solid #e5e7eb}
.copy-button{width:100%;height:42px;border:0;border-radius:8px;background:#111827;color:#fff;font-size:13px;font-weight:600;cursor:pointer}.copy-button:disabled{opacity:.45;cursor:not-allowed}
