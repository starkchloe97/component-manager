...
<template>
  <div v-if="node.type === 'section'" class="editor-node editor-node--section"
    ...
    <InsertionPoint v-if="rootNode && !isLastRootNode" label="Add after" :visible="isSelected"
      @activate="openInsertion" />
    <div v-if="isSelected" class="node-toolbar" @click.stop><span class="node-toolbar-label">{{ nodeLabel }}</span>
      <GripVertical :size="16" class="drag-grip" /><button ref="addTrigger" type="button" class="toolbar-icon-button"
        title="Add after section" aria-label="Add after section" @click="openAdd($event, 'after')">
        <Plus :size="15" />
      </button><button type="button" class="toolbar-icon-button" title="Duplicate section"
        aria-label="Duplicate section" @click="duplicateNode(node.id)">
        <Copy :size="15" />
      </button><button type="button" class="toolbar-icon-button toolbar-icon-danger" title="Delete section"
        aria-label="Delete section" @click="removeNode">
        <Trash2 :size="15" />
      </button>
    </div>
  </div>
  ...
  <div v-else-if="node.type === 'column' || node.type === 'container'" class="editor-node"
    ...
    <div :class="node.type === 'container' ? 'editor-container' : 'editor-column'" :style="node.styles">
      ...
      <EditorNode v-for="child in node.children" :key="child.id" :node="child" :parent-id="node.id"
        :parent-type="node.type" />
      <button v-if="isSelected && !isEmptyHost" ref="addTrigger" type="button" class="inside-add"
        @click.stop="openAdd($event, 'inside')">
        <Plus :size="14" />Add inside
      </button>
    </div>
    <InsertionPoint v-if="rootNode && !isLastRootNode" label="Add after" :visible="isSelected"
      @activate="openInsertion" />
    <div v-if="isSelected" class="node-toolbar" @click.stop><span class="node-toolbar-label">{{ nodeLabel }}</span>
      <GripVertical :size="16" class="drag-grip" /><button v-if="parentId" type="button" class="toolbar-icon-button"
        :aria-label="`Select ${parentLabel}`" @click="selectParent">
        <ArrowUp :size="15" />
      </button><button type="button" class="toolbar-icon-button" title="Add inside" aria-label="Add inside"
        @click="openAdd($event, 'inside')">
        <Plus :size="15" />
      </button><button type="button" class="toolbar-icon-button" title="Duplicate" aria-label="Duplicate"
        @click="duplicateNode(node.id)">
        <Copy :size="15" />
      </button><button type="button" class="toolbar-icon-button toolbar-icon-danger" title="Delete" aria-label="Delete"
        @click="removeNode">
        <Trash2 :size="15" />
      </button>
    </div>
  ...
  <div v-else-if="node.type === 'component'" class="editor-node" :class="{ 'editor-node--selected': isSelected }"
    ...
    <div v-if="isSelected" class="node-toolbar" @click.stop><span class="node-toolbar-label">{{ nodeLabel }}</span>
      <GripVertical :size="16" class="drag-grip" /><button ref="addTrigger" type="button" class="toolbar-icon-button"
        aria-label="Add after component" @click="openAdd($event, 'after')">
        <Plus :size="15" />
      </button><button type="button" class="toolbar-icon-button" aria-label="Duplicate component"
        @click="duplicateNode(node.id)">
        <Copy :size="15" />
      </button><button type="button" class="toolbar-icon-button toolbar-icon-danger" aria-label="Delete component"
        @click="removeNode">
        <Trash2 :size="15" />
      </button>
    </div>
  ...
  <div v-if="isSelected && isLeaf" class="node-toolbar node-toolbar--leaf" :style="leafToolbarStyle" @click.stop>
    <span>{{ nodeLabel }}</span>
    <GripVertical :size="16" class="drag-grip" /><button ref="addTrigger" type="button" class="toolbar-icon-button"
      aria-label="Add after element" @click="openAdd($event, 'after')">
      <Plus :size="15" />
    </button><button type="button" class="toolbar-icon-button" aria-label="Duplicate" @click="duplicateNode(node.id)">
      <Copy :size="15" />
    </button><button type="button" class="toolbar-icon-button toolbar-icon-danger" aria-label="Delete"
      @click="removeNode">
      <Trash2 :size="15" />
    </button>
  </div>
  <AddMenu :open="showAdd" :anchor="addTrigger" :title="addTitle" @select="handleAdd" @close="showAdd = false" />
  <SectionLayoutPicker v-if="showSectionPicker"
    ...
    @select="addSectionWithLayout" @close="showSectionPicker = false" />
  <SectionLayoutPicker v-if="showContainerPicker"
    :title="addMode === 'inside' ? 'Add container inside' : 'Add container after'"
    :description="addMode === 'inside' ? 'Choose a layout for the nested container.' : 'Choose a layout for the sibling container.'"
    :allow-empty-container="true" @select="addContainerWithLayout" @close="showContainerPicker = false" />
</template>
...
.editor-node:hover>.editor-section,
.editor-node:hover>.editor-column,
.editor-node:hover>.editor-container {
  border-color: rgba(230, 164, 244, .7);
  background: rgba(255, 246, 255, .45)
}
...
.empty-host button:hover,
.empty-host button:focus-visible,
.inside-add:hover,
.inside-add:focus-visible {
  outline: 0;
  border-color: #b9159d;
  background: #fff4fe;
  color: #8f0070
