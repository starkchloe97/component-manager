...
<template>
  <aside class="elementor-inspector" aria-label="Element settings">
    <header class="inspector-title"><strong>Edit {{ normalizedName }}</strong><span>{{ isNode ? 'Builder node' :
        'Component element' }}</span></header>
    ...
    <div v-if="entity" class="inspector-body">
      <template v-if="activeTab === 'content'">
        <section v-if="isNode && ['heading', 'text', 'button'].includes(selectedNode.type)" class="control-group">
          <h3>Content</h3><label>Text<textarea v-if="selectedNode.type === 'text'" rows="4"
              :value="selectedNode.props.text" @input="updateProp('text', $event.target.value)" /><input v-else
              :value="selectedNode.props.text" @input="updateProp('text', $event.target.value)" /></label><label
            v-if="selectedNode.type === 'heading'">HTML Tag<select :value="selectedNode.props.tag || 'h2'"
              @change="updateProp('tag', $event.target.value)">
              <option v-for="tag in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']" :key="tag">{{ tag }}</option>
            </select></label><label v-if="selectedNode.type === 'button'">Link<input :value="selectedNode.props.href"
              @input="updateProp('href', $event.target.value)" /></label>
        </section>
        <section v-else-if="isNode && selectedNode.type === 'image'" class="control-group">
          <h3>Image</h3><label>Source URL<input :value="selectedNode.props.src"
              @input="updateProp('src', $event.target.value)" /></label><label>Alt text<input
              :value="selectedNode.props.alt" @input="updateProp('alt', $event.target.value)" /></label>
        </section>
        <section v-else-if="selectedElement?.editableText" class="control-group">
          <h3>Text</h3><textarea rows="6" :value="contentValue()" @input="updateContent($event.target.value)" /><button
            v-if="getContentEntry(selectedElement.componentId, selectedElement.contentSelector)" class="reset-text"
            type="button" @click="resetText">Reset text</button>
        </section>
        <section v-else class="empty-state">
          <Type :size="28" /><strong>No editable content</strong><span>Select text, an image, or a button to edit its
            content.</span>
        </section>
      </template>
      <template v-else-if="activeTab === 'layout'">
        <section class="control-group">
          ...
          <div v-for="field in layoutFields" :key="field.key" class="field"><label :for="`layout-${field.key}`">{{
              field.label }}</label><select v-if="field.options" :id="`layout-${field.key}`" :value="current(field.key)"
            ...
            <div v-else class="value-control"><input :id="`layout-${field.key}`" :value="current(field.key)"
                ...
                v-if="current(field.key)" type="button" @click="resetStyle(field.key)">×</button></div>
          </div>
        </section>
        <section v-if="isFlex || isGrid" class="control-group">
          <h3>{{ isFlex ? 'Items' : 'Grid' }}</h3>
          <div v-for="field in isFlex ? flexFields : gridFields" :key="field.key" class="field"><label>{{ field.label
              }}</label><select v-if="field.options" :value="current(field.key)"
              @change="updateStyle(field.key, $event.target.value)">
              <option v-for="option in field.options" :key="option" :value="option">{{ option }}</option>
            </select><input v-else :value="current(field.key)" :placeholder="field.placeholder"
      ...
      <template v-else-if="activeTab === 'style'">
        <section class="control-group">
          <h3>Typography</h3>
          <div v-for="field in typographyFields" :key="field.key" class="field"><label>{{ field.label }}</label><select
              v-if="field.options" :value="current(field.key)" @change="updateStyle(field.key, $event.target.value)">
              <option v-for="option in field.options" :key="option" :value="option">{{ option || 'Default' }}</option>
            </select><input v-else :value="current(field.key)" :placeholder="field.placeholder"
              @input="updateStyle(field.key, $event.target.value)" /></div>
        </section>
        <section class="control-group">
          <h3>Appearance</h3>
          <div v-for="field in appearanceFields" :key="field.key" class="field"><label>{{ field.label }}</label>
            <div v-if="field.color" class="color-control"><input type="color" :value="current(field.key) || '#ffffff'"
                @input="updateStyle(field.key, $event.target.value)" /><input :value="current(field.key)"
                placeholder="#ffffff" @input="updateStyle(field.key, $event.target.value)" /></div><select
              v-else-if="field.options" :value="current(field.key)"
              @change="updateStyle(field.key, $event.target.value)">
              <option v-for="option in field.options" :key="option" :value="option">{{ option || 'Default' }}</option>
            </select><input v-else :value="current(field.key)" :placeholder="field.placeholder"
      ...
      <template v-else-if="activeTab === 'advanced'">
        <section class="control-group">
          ...
          <div v-for="box in [['padding', 'Padding'], ['margin', 'Margin']]" :key="box[0]" class="spacing">
            <div class="field"><label>{{ box[1] }}</label><input :value="current(box[0])" placeholder="All"
                @input="updateStyle(box[0], $event.target.value)" /></div>
            <div class="side-grid"><label v-for="side in ['Top', 'Right', 'Bottom', 'Left']" :key="side">{{ side }}<input
                  :value="current(`${box[0]}${side}`)"
                  @input="updateStyle(`${box[0]}${side}`, $event.target.value)" /></label></div>
          </div>
        </section>
        <section class="control-group">
          <h3>Layout</h3>
          <div
            ...
            :key="field.key" class="field"><label>{{ field.label }}</label><input :value="current(field.key)"
              :placeholder="field.placeholder" @input="updateStyle(field.key, $event.target.value)" /></div>
        </section>
      </template>
    </div>
    <div v-else class="empty-state">
      <Box :size="30" /><strong>Select an element</strong><span>Click a component element or builder container to edit
        it.</span>
    </div>
  </aside>
</template>
