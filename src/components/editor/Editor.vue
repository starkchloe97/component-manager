      ...
      <main class="editor-workspace">
        <header class="workspace-bar">
          ...
          <div class="workspace-actions">
            <button class="inspector-toggle" :class="{ active: inspectorOpen }" type="button" :title="inspectorOpen ? 'Close settings' : 'Open settings'" :aria-label="inspectorOpen ? 'Close settings' : 'Open settings'" @click="inspectorOpen = !inspectorOpen"><span class="toggle-icon"><Menu v-if="!inspectorOpen" :size="18" /><X v-else :size="18" /></span></button>
...
<style scoped>
.editor{position:fixed;inset:0;z-index:10000;background:#fff;color:#20252b;font-family:Arial,Helvetica,sans-serif}.editor-shell{position:absolute;inset:0;display:flex;min-width:0;overflow:hidden;background:#fff}.editor-inspector{position:relative;z-index:20;width:320px;flex:0 0 320px;overflow:hidden;transition:width .25s ease,flex-basis .25s ease}.editor:not(.inspector-open) .editor-inspector{width:0;flex-basis:0}.editor-workspace{position:relative;display:flex;flex:1;min-width:0;flex-direction:column;background:#f7f7f7;overflow:hidden}.workspace-bar{height:54px;flex:0 0 54px;display:flex;align-items:center;gap:13px;padding:0 17px;border-bottom:1px solid #e4e6e9;background:#fff}.brand-button,.workspace-actions button{display:grid;place-items:center;width:32px;height:32px;padding:0;border:0;border-radius:3px;background:transparent;color:#30343a;cursor:pointer}.brand-button:hover,.workspace-actions button:hover:not(:disabled){background:#f2f3f5;color:#93003f}.inspector-toggle{position:relative;overflow:hidden}.inspector-toggle .toggle-icon{display:grid;place-items:center;width:18px;height:18px;transition:transform .2s ease,opacity .2s ease}.inspector-toggle.active .toggle-icon{transform:rotate(0deg)}.workspace-actions button:disabled{cursor:default;opacity:.35}.workspace-name{display:flex;align-items:center;gap:7px;min-width:0;font-size:13px;font-weight:700}.workspace-name svg{color:#93003f}.workspace-actions{display:flex;align-items:center;gap:3px;margin-left:auto}.toolbar-divider{width:1px;height:24px;margin:0 5px;background:#e1e3e6}
@media(max-width:900px){.editor-inspector{position:absolute;top:0;bottom:0;left:0;z-index:50;width:min(375px,92vw);flex-basis:min(375px,92vw);box-shadow:12px 0 28px rgba(0,0,0,.14);transition:transform .25s ease;transform:translateX(0)}.editor:not(.inspector-open) .editor-inspector{width:min(375px,92vw);flex-basis:min(375px,92vw);transform:translateX(-100%)}.workspace-actions button:nth-child(2),.workspace-actions .toolbar-divider{display:none}}
